...
import { nobleSupportSkillModFactories } from "./noble_support_factories";
import { v } from "./types";

/**
 * Get mods for a noble support skill at the specified tier, rank, and value.
 *
 * @param skillName - Name of the noble support skill
 * @param tier - Tier 0-2 (lower is better, tier 0 has best values)
 * @param rank - Rank 1-5 (higher is better, rank 5 is max)
 * @param value - Specific value within the tier's range (e.g., -5 for tier 0 range -7 to -5)
 * @returns Array of Mod objects
 */
export const getNobleSupportSkillMods = (
  skillName: NobleSupportSkillName,
  tier: 0 | 1 | 2,
  rank: 1 | 2 | 3 | 4 | 5,
  value: number,
): Mod[] => {
  // Get skill data from generated noble support skills
  const skill = NobleSupportSkills.find((s) => s.name === skillName) as
    | BaseNobleSupportSkill
    | undefined;
  if (skill === undefined) {
    throw new Error(`Noble support skill "${skillName}" not found`);
  }
  ...
  const factory = nobleSupportSkillModFactories[skillName];
  const factoryMods = factory !== undefined ? factory(tier, value, vals) : [];

  // Always include rank-based damage mod (present in all noble supports)
